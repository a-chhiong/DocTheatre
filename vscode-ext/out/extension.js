...
// src/resolvers/openapi.ts
function preprocessOpenApiRefs(rawContent, basePath) {
  try {
    const isJson = basePath.endsWith(".json");
    const spec = isJson ? JSON.parse(rawContent) : load(rawContent);
    if (!spec || typeof spec !== "object") {
      return rawContent;
    }
    const visited = /* @__PURE__ */ new Set([basePath]);
...
function resolveRefNode(node, baseDir, visited) {
  ...
  if (node !== null && typeof node === "object") {
    ...
    if (typeof obj["$ref"] === "string") {
      ...
      const resolvedPath = path2.resolve(baseDir, filePart);
      ...
      try {
        const fileContent = fs2.readFileSync(resolvedPath, "utf8");
        ...
        const inlined = resolveRefNode(fileSpec, path2.dirname(resolvedPath), nextVisited);
        if (fragment && fragment !== "#") {
          const parts = fragment.replace(/^#\/?/, "").split("/");
          let sub = inlined;
          for (const part of parts) {
            if (sub !== null && typeof sub === "object") {
              sub = sub[part.replace(/~1/g, "/").replace(/~0/g, "~")];
            } else {
              sub = void 0;
              break;
            }
...
var PreviewPanel = class _PreviewPanel {
  // ── Constructor ─────────────────────────────────────────────────────────────
  constructor(panel, extensionUri, initialDocument) {
    this._disposables = [];
  ...
  static createOrShow(extensionUri) {
    ...
    if (_PreviewPanel.current) {
      _PreviewPanel.current._panel.reveal(column, true);
      return;
    }
    ...
    _PreviewPanel.current = new _PreviewPanel(panel, extensionUri, initialDocument);
  }
...
function activate(context) {
  ...
  const refLinkProvider = {
    provideDocumentLinks(document) {
      ...
      while ((match = regex.exec(text)) !== null) {
        ...
        const docDir = path4.dirname(document.uri.fsPath);
        const targetPath = path4.resolve(docDir, filePart);
        const link = new vscode2.DocumentLink(range, vscode2.Uri.file(targetPath));
  ...
  context.subscriptions.push(
    vscode2.languages.registerDocumentLinkProvider(
      [
