    ...
    for (const table of tables) {
      ...
      for (const field of table.fields) {
        ...
        if (field.note) {
          ...
          let rawNoteHtml = `<button class="dbdocs-note-icon-btn js-open-note-modal" title="View Note" data-modal-target="${modalId}">${mdIcon}</button>`;
          rawNoteHtml += `<dialog id="${modalId}" class="dbdocs-modal js-note-modal">`;
          rawNoteHtml += `<div class="dbdocs-modal-header"><h4>Note: ${field.name}</h4><button class="dbdocs-modal-close js-close-note-modal">&times;</button></div>`;
